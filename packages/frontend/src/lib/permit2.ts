import { AllowanceProvider, AllowanceTransfer, MaxAllowanceTransferAmount, PERMIT2_ADDRESS, type PermitSingle } from '@uniswap/permit2-sdk'
...
export const getAllowance = async (wallet: Wallet, spender: string, token: string) => {
...
export const authorize = async (wallet: Wallet, token: string) => {
  const permit2ContractAbi = [
    'function approve(address spender,uint amount)',
  ]
  const permit2Contract = new Contract(token, permit2ContractAbi, wallet.signer)
...
export const handlePermit = async (wallet: Wallet, token: string, spender: string) => {
  ...
  const signature = await signTypedData(wallet.signer, domain, types, values)
  ...
  const permitContract = new Contract(PERMIT2_ADDRESS, permitAbi, wallet.signer)
  return await permitContract.permit(account, permitSingle, signature)
}
